    ...
    for subset in subsets:
        ...
        for sample in progress:
            ...
            for k, v in log_output.items():
                ...
                if k == 'src_target_hypo_str':
                    src_target_hypo_strs.extend(log_output['src_target_hypo_str'])
                    continue

                extra_meters[k].update(v)
        if args.distributed_rank == 0:
          extra_meters['bleu'].update(get_bleu_score(args, trainer, subset, src_target_hypo_strs), n=1)
...
def get_bleu_score(args, trainer, subset, src_target_hypo_strs):
    ...
    if args.valid_decoding_path is not None:
        ...
        with open(valid_sys_path, 'w', encoding="utf8") as f:
