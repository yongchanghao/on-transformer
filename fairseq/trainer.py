...
class Trainer(object):
    ...
    def _prepare_sample(self, sample, force_float=False):
        ...
        if self.args.fp16 and not force_float:
    ...
    def _build_generator(self):
        ...
        try:
            self._src_dict = getattr(self.task, 'source_dictionary', None)
        except NotImplementedError:
            self._src_dict = None
        self._tgt_dict = self.task.target_dictionary

    def _get_decoding(self, sample):
        src_target_hypo_str = []
        prefix_tokens = None
        if self.args.prefix_size > 0:
            prefix_tokens = sample['target'][:, :self.args.prefix_size]

        hypos = self.task.inference_step(self._generator, [self.model], sample, prefix_tokens)

        for i, sample_id in enumerate(sample['id'].tolist()):
            assert self.args.nbest == 1, 'Online bleu computation needs args.nbest == 1'

            has_target = sample['target'] is not None

            # Remove padding
            src_tokens = utils.strip_pad(sample['net_input']['src_tokens'][i, :], self._tgt_dict.pad())
            target_tokens = None
            if has_target:
                target_tokens = utils.strip_pad(sample['target'][i, :], self._tgt_dict.pad()).int().cpu()

            # Either retrieve the original sentences or regenerate them from tokens.

            src_str = self._src_dict.string(src_tokens, self.args.remove_bpe)

            if has_target:
                target_str = self._tgt_dict.string(target_tokens, self.args.remove_bpe, escape_unk=True)

            # Process top predictions
            for i, hypo in enumerate(hypos[i][:min(len(hypos), self.args.nbest)]):
                hypo_str = self._tgt_dict.string(hypo['tokens'].int().cpu(), self.args.remove_bpe)

                src_target_hypo_str.append((sample_id, src_str, target_str, hypo_str))

        return src_target_hypo_str
