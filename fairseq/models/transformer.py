...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--encoder-combination', choices=["linear", "dynamic", "dense"],
                            help='use combination to express encoder output')
        parser.add_argument('--decoder-combination', choices=["linear", "dynamic", "dense"],
                            help='use combination to express decoder output')
        parser.add_argument('--encoder-rezero', action='store_true', default=False,
                            help='use rezero for encoder layers')
        # fmt: on
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if args.encoder_rezero:
            self.rezero_weights = nn.ParameterList([
                nn.Parameter(torch.zeros(1)) for _ in range(args.encoder_layers)
            ])
    ...
    def forward(self, src_tokens, src_lengths, cls_input=None, return_all_hiddens=False, **unused):
        ...
        if self.encoder_combination in ['dynamic', 'linear']:
            hiddens = []
        elif self.encoder_combination in ['dense']:
            prev_sum = x.clone()
        else:
            hiddens = None
        ...
        for idx, layer in enumerate(self.layers):
            ...
            rezero_weight = self.rezero_weights[idx] if self.encoder_rezero else None
            dropout_probability = random.uniform(0, 1)
            if not self.training or (dropout_probability >= self.encoder_layerdrop):
                if self.encoder_combination == 'dense':
                    x = layer(prev_sum, encoder_padding_mask)
                else:
                    x = layer(x, encoder_padding_mask)
                if self.encoder_combination in ['dynamic', 'linear']:
                    hiddens.append(x)
                elif self.encoder_combination == 'dense':
                    prev_sum = torch.dropout(prev_sum + x, self.dropout, self.training)
                if return_all_hiddens:
                    encoder_states.append(x)

        if self.encoder_combination == "dynamic":
            ...
            x = torch.dropout(acc_x + self.weight_ffn(acc_x), self.dropout, self.training)
        elif self.encoder_combination == "linear":
            assert torch.equal(x, hiddens[-1])
            acc_x = torch.zeros_like(x)
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.decoder_combination = args.decoder_combination
        if self.decoder_combination in ['dynamic', 'linear']:
            self.weight_ffn = nn.Sequential(
                nn.Linear(embed_dim, args.decoder_ffn_embed_dim),
                nn.ReLU(),
                nn.Linear(args.decoder_ffn_embed_dim, embed_dim),
            )
        if self.decoder_combination == 'dynamic':
            self.proj = nn.ModuleList([
                nn.Sequential(
        ...
        if self.decoder_combination == 'linear':
            self.weights = nn.ParameterList([
                nn.Parameter(torch.randn(1, 1, embed_dim), requires_grad=True)
                for _ in range(args.decoder_layers)
            ])
    ...
    def extract_features(
        ...
        if self.decoder_combination in ['dynamic', 'linear']:
            hiddens = []
        elif self.decoder_combination in ['dense']:
            prev_sum = x.clone()
        else:
            hiddens = None
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if not self.training or (dropout_probability >= self.decoder_layerdrop):
                if self.decoder_combination == 'dense':
                    x, layer_attn = layer(
                        prev_sum,
                        encoder_state,
        ...
        if self.decoder_combination == 'dynamic':
            ...
            for i, layer in enumerate(self.proj):
                acc_x += layer(catted_hidden) * hiddens[i]
            x = torch.dropout(acc_x + self.weight_ffn(acc_x), self.dropout, self.training)
        elif self.decoder_combination == 'linear':
            assert torch.equal(x, hiddens[-1])
            acc_x = torch.zeros_like(x)
            for i, weight in enumerate(self.weights):
                acc_x += weight * hiddens[i]
            x = torch.dropout(acc_x + self.weight_ffn(acc_x), self.dropout, self.training)
        elif self.decoder_combination == 'dense':
            x = prev_sum
...
def base_architecture(args):
    ...
    args.encoder_combination = getattr(args, "encoder_combination", False)
    args.decoder_combination = getattr(args, "decoder_combination", False)
